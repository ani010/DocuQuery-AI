...
const AppState = {
  ...
  uploadedFiles: JSON.parse(localStorage.getItem('docuquery_files') || '[]').filter(
    s => !['financial_report_2025.pdf', 'employee_handbook.txt', 'retail_catalog_v2.csv'].includes(s.filename)
  ), // Stores list of uploaded files
};
...
function setupSidebarSessions() {
  ...
  if (AppState.uploadedFiles.length === 0) {
    ...
    emptyLi.className = 'session-empty-state';
    emptyLi.style.padding = '12px 16px';
    emptyLi.style.color = 'var(--color-text-muted)';
    emptyLi.style.fontSize = '0.85rem';
    emptyLi.style.textAlign = 'center';
    emptyLi.textContent = 'No documents uploaded yet';
    elements.sessionList.appendChild(emptyLi);
    return;
  }

  AppState.uploadedFiles.forEach(session => {
    ...
    li.innerHTML = `
      ...
      <button class="delete-btn" aria-label="Delete document" type="button">
    ...
    deleteBtn.addEventListener('click', (e) => {
      ...
      deleteSidebarSession(session.id);
    });
...
function deleteSidebarSession(sessionId) {
  ...
  AppState.uploadedFiles = AppState.uploadedFiles.filter(s => s.id !== sessionId);

  // Persist to storage
  localStorage.setItem('docuquery_files', JSON.stringify(AppState.uploadedFiles));

  // If the deleted session was the active session, reset chat view
...
async function uploadFile(file) {
  ...
  try {
    ...
    // Add to dynamic session list if not already present
    const fileType = file.name.split('.').pop().toLowerCase();
    const existingSession = AppState.uploadedFiles.find(s => s.filename === file.name);
    if (!existingSession) {
      const newSession = {
      ...
      AppState.uploadedFiles.push(newSession);
      AppState.activeSessionId = newSession.id;
    } else {
      AppState.activeSessionId = existingSession.id;
    }
